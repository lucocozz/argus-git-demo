...
ARGUS_OPTIONS(
    ...
    OPTION_FLAG('q', "quiet", HELP("Suppress diff output")),
    OPTION_FLAG('\0', "oneline", HELP("Show each commit on a single line")),
    OPTION_FLAG('\0', "graph", HELP("Draw a text-based graphical representation of the commit history")),
    OPTION_FLAG('\0', "stat", HELP("Generate a diffstat")),
    OPTION_FLAG('\0', "numstat", HELP("Show number of added and deleted lines in decimal notation")),
    OPTION_FLAG('\0', "shortstat", HELP("Output only the last line of the stat format")),
    OPTION_FLAG('\0', "name-only", HELP("Show only names of changed files")),
    OPTION_FLAG('\0', "name-status", HELP("Show only names and status of changed files")),
    OPTION_FLAG('p', "patch", HELP("Generate patch")),
    OPTION_FLAG('\0', "abbrev-commit", HELP("Show only a partial prefix of the commit object name")),
    OPTION_FLAG('\0', "source", HELP("Show source")),
    OPTION_FLAG('\0', "use-mailmap", HELP("Use mail map file")),
    OPTION_FLAG('\0', "mailmap", HELP("Alias of --use-mailmap")),
    OPTION_FLAG('\0', "clear-decorations", HELP("Clear all previously-defined decoration filters")),
    OPTION_INT('n', "max-count", HELP("Limit the number of commits to output"), HINT("number")),
    OPTION_INT('\0', "skip", HELP("Skip number commits before starting to show the commit output"), HINT("number")),
    OPTION_STRING(
        '\0', "pretty",
        HELP("Pretty-print the contents of the commit logs"),
        HINT("format"),
        VALIDATOR(V_CHOICE_STR("oneline", "short", "medium", "full", "fuller", "format")),
        DEFAULT("medium")),
    OPTION_STRING(
        '\0', "format",
        HELP("Specify which information is shown"),
        HINT("format")),
    OPTION_STRING(
        '\0', "decorate-refs", 
    ...
    OPTION_STRING(
        ...
        HINT("format"),
        VALIDATOR(V_CHOICE_STR("short", "full", "auto", "no")),
        DEFAULT("short"),
        FLAGS(FLAG_OPTIONAL)),
    OPTION_STRING(
        'L', NULL,
        HELP("Trace the evolution of line range <start>,<end> or function :<funcname> in <file>"),
        HINT("range:file")),
    
    POSITIONAL_MANY_STRING(
        "revision",
        HELP("Show commits reachable from given commits"),
        FLAGS(FLAG_OPTIONAL)),
)
...
int log_handler(argus_t *argus, void *data)
{
    ...
    const git_commit_t commits[] = {
        {"abc1234", "Add new feature for user authentication", "John Doe", "2024-01-15 10:30:45", "john.doe@example.com"},
        {"def5678", "Fix bug in payment processing", "Jane Smith", "2024-01-14 15:22:10", "jane.smith@example.com"}, 
        {"ghi9012", "Update documentation for API endpoints", "Bob Johnson", "2024-01-13 09:15:33", "bob.johnson@example.com"},
        {"jkl3456", "Refactor database connection logic", "Alice Brown", "2024-01-12 14:45:20", "alice.brown@example.com"},
        {"mno7890", "Initial commit", "Developer", "2024-01-10 08:00:00", "dev@example.com"}
    };
    
    int total_count = 5;
    int max_count = argus_is_set(argus, "max-count") ? argus_get(argus, "max-count").as_int : 0;
    int skip = argus_is_set(argus, "skip") ? argus_get(argus, "skip").as_int : 0;
    ...
    // Display commits
    for (int i = start_index; i < end_count; i++) {
        ...
        bool oneline = argus_get(argus, "oneline").as_bool;
        const char *pretty = argus_get(argus, "pretty").as_string;
        const char *custom_format = argus_is_set(argus, "format") ? argus_get(argus, "format").as_string : NULL;
        
        if (oneline || (pretty && strcmp(pretty, "oneline") == 0)) {
            print_commit_oneline(commit, argus, display_hash);
        } else if (custom_format) {
            // Custom format simulation
