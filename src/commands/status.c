...
static void print_porcelain_status(argus_t *argus, const git_file_status_t files[], int count)
{
    ...
    const char *ignored_mode = argus_is_set(argus, "ignored") ? argus_get(argus, "ignored").as_string : NULL;
...
int status_handler(argus_t *argus, void *data)
{
    ...
    // Simulate Git status data with structures
    const git_file_status_t files[] = {
    ...
    const char *porcelain = argus_is_set(argus, "porcelain") ? argus_get(argus, "porcelain").as_string : NULL;
