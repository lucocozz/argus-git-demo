...
ARGUS_OPTIONS(
    remote_add_options,
    HELP_OPTION(),
    OPTION_FLAG('f', "fetch", HELP("Immediately fetch from the new remote")),
    OPTION_FLAG(0, "tags", HELP("Import every tag from the remote with --fetch")),
    OPTION_FLAG(0, "no-tags", HELP("Do not import tags from the remote with --fetch")),
    OPTION_ARRAY_STRING('t', NULL,
        ...
        HINT("branch")),
    OPTION_STRING('m', NULL, HELP("Set the default branch for the remote")),
    POSITIONAL_STRING("name", HELP("Name of the remote")),
    POSITIONAL_STRING("url", HELP("URL of the remote repository"), VALIDATOR(V_REGEX(ARGUS_RE_URL))),
)
...
int remote_add_handler(argus_t *argus, void *data) {
    ...
    const char *master_branch = argus_get(argus, "m").as_string;
    if (master_branch)
