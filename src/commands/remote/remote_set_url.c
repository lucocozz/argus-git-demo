...
ARGUS_OPTIONS(
    ...
    POSITIONAL_STRING(
        "oldurl",
        HELP("Old URL to replace (for --delete)"),
        FLAGS(FLAG_OPTIONAL)
    ),
)
...
int remote_set_url_handler(argus_t *argus, void *data) {
    ...
    if (add) {
        printf("Adding URL '" COLOR_BLUE("%s") "' to remote '" COLOR_CYAN("%s") "'\n", newurl, name);
    } else if (delete) {
        printf("Deleting URL '" COLOR_BLUE("%s") "' from remote '" COLOR_CYAN("%s") "'\n", newurl, name);
    } else {
        printf("Setting " COLOR_BLUE("%s") " URL for remote '" COLOR_CYAN("%s") "' to '" COLOR_BLUE("%s") "'\n", 
