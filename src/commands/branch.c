...
ARGUS_OPTIONS(
    ...
    GROUP_START("Generic options"),
        OPTION_FLAG('v', "verbose", HELP("Show hash and subject, give twice for upstream branch")),
        ...
        OPTION_STRING('t', "track",
            ...
            FLAGS(FLAG_OPTIONAL)),
        OPTION_STRING('u', "set-upstream-to", HELP("Change the upstream info"), HINT("upstream")),
        ...
        OPTION_STRING('\0', "contains", HELP("Print only branches that contain the commit"), HINT("commit")),
        OPTION_STRING('\0', "no-contains", HELP("Print only branches that don't contain the commit"), HINT("commit")),
    ...
    GROUP_START("Specific git-branch actions"),
        ...
        OPTION_STRING('\0', "merged", HELP("Print only branches that are merged"), HINT("commit")),
        OPTION_STRING('\0', "no-merged", HELP("Print only branches that are not merged"), HINT("commit")),
        OPTION_FLAG('\0', "column", HELP("List branches in columns")),
        OPTION_FLAG('\0', "sort", HELP("Field name to sort on")),
        OPTION_STRING('\0', "points-at", HELP("Print only branches of the object"), HINT("object")),
        ...
        OPTION_STRING('\0', "format", HELP("Format to use for the output")),
    GROUP_END(),
...
static int handle_branch_creation(argus_t *argus, const char *branchname, const char *start_point)
{
    ...
    if (track_mode) printf("Branch '" COLOR_GREEN("%s") "' set up to track upstream (mode: %s).\n", branchname, track_mode);
    return 0;
}
...
static void display_local_branches(const git_branch_t *branches, int count, bool verbose, bool merged, bool no_merged)
{
    for (int i = 0; i < count; i++) {
        ...
        if (verbose) printf("%s" COLOR_GREEN("%-10s") " " COLOR_YELLOW("%s") " %s\n", prefix, branch->name, branch->hash, branch->message);
        else printf("%s" COLOR_GREEN("%s") "\n", prefix, branch->name);
    }
}
...
static void display_remote_branches(const git_branch_t *branches, int count, bool verbose)
{
    for (int i = 0; i < count; i++) {
        ...
        if (verbose) printf("  " COLOR_CYAN("%-20s") " " COLOR_YELLOW("%s") " %s\n", branch->name, branch->hash, branch->message);
        else printf("  " COLOR_CYAN("%s") "\n", branch->name);
    }
}
...
static void display_branch_list(argus_t *argus, bool verbose, bool remotes, bool all)
{
    ...
    if (contains) printf("Branches containing commit '%s':\n", contains);
    ...
    if (!remotes) display_local_branches(branches, branch_count, verbose, merged, no_merged);
    if (all || remotes) display_remote_branches(remote_branches, remote_branch_count, verbose);
}
...
int branch_handler(argus_t *argus, void *data)
{
    ...
    if (has_deletion_flags(argus) && branchname) return handle_branch_deletion(argus, branchname);
    if (has_move_copy_flags(argus) && branchname) return handle_branch_move_copy(argus, branchname, start_point);
    if (has_upstream_flags(argus) && branchname) return handle_branch_upstream(argus, branchname);
    if (branchname && !has_action_flags(argus)) return handle_branch_creation(argus, branchname, start_point);
