...
ARGUS_OPTIONS(
    ...
    GROUP_START("Mode options"),
        ...
        OPTION_FLAG('A', "all", HELP("Add changes from all tracked and untracked files")),
        OPTION_FLAG('\0', "ignore-removal", HELP("Ignore paths removed in the working tree (same as --no-all)")),
    ...
    GROUP_START("Error handling"),
        OPTION_FLAG('\0', "ignore-errors", HELP("Just skip files which cannot be added because of errors")),
        OPTION_FLAG('\0', "ignore-missing", HELP("Check if - even missing - files are ignored in dry run")),
    GROUP_END(),
    
    GROUP_START("Advanced options"),
        OPTION_FLAG('\0', "sparse", HELP("Allow updating entries outside of the sparse-checkout cone")),
        OPTION_STRING('\0', "chmod", HELP("Override the executable bit of the listed files"),
                      VALIDATOR(V_CHOICE_STR("+x", "-x"))),
        OPTION_STRING('\0', "pathspec-from-file", HELP("Read pathspec from file")),
        OPTION_FLAG('\0', "pathspec-file-nul", HELP("With --pathspec-from-file, pathspec elements are separated with NUL character")),
    GROUP_END(),
...
static void process_pathspecs(argus_t *argus)
{
    ...
    const char *chmod_mode = argus_get(argus, "chmod").as_string;
...
int add_handler(argus_t *argus, void *data)
{
    ...
    result = handle_interactive_modes(argus);
    if (result != -1)
        return result;
    
    result = handle_special_modes(argus);
    if (result != -1)
        return result;
